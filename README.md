...
- `config_p65.json` 保存全部配置，包括 SSH、远端仿真目录、测试参数、窗口状态和最近输出目录。
...
### ui_settings.json 是什么

新版已经不再使用 `ui_settings.json`。

以前它用于保存指令数、仿真时间、窗口大小和最近输出目录。现在这些内容统一保存到：

```text
config_p65.json
```

如果本地旧目录里还有 `ui_settings.json`，可以直接删除。
